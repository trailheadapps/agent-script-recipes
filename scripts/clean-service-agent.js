...
// Get agent files from command arguments
...
// Remove default_agent_user value and replace it with a placeholder
let restoredCount = 0;
for (const filePath of agentFiles) {
    ...
    if (AGENT_USER_REGEX.test(content)) {
        ...
        console.log(
            `Restored default_agent_user placeholder in: ${path.basename(filePath)}`
        );
        restoredCount++;
    }
}
