...
const VALIDATOR_URL = 'https://git.soma.salesforce.com/pages/chatbots/module-agentscript/agents';
const PROFILE_DIR = resolve(process.env.HOME, '.agentscript-validator-session');
const REPO_ROOT = resolve(__dirname, '..');
const SETTLE_MS = 2500;

function findAgentFiles() {
  const out = execSync('find force-app -name "*.agent" | sort', { cwd: REPO_ROOT, encoding: 'utf8' });
  return out.trim().split('\n').filter(Boolean).map(f => resolve(REPO_ROOT, f));
}

const filesArgIdx = process.argv.indexOf('--files');
const agentFiles = filesArgIdx !== -1
...
async function scrapeIssuesDOM(page) {
  return page.evaluate(() => {
    ...
    allButtons.forEach(btn => {
      ...
      const ruleSpan = Array.from(spans).find(s => s.textContent.includes('agentscript-lint') || s.textContent.includes('-lint'));
      if (!ruleSpan) return;
      // spans[0] = dot, spans[1] = message text, ruleSpan = rule, last span = line:col
      const dotStyle  = spans[0] ? spans[0].getAttribute('style') || '' : '';
      const message   = spans[1] ? spans[1].textContent.trim() : '';
      const rule      = ruleSpan.textContent.trim();
      const locSpan   = spans[spans.length - 1];
      const loc       = locSpan && /^\d+:\d+$/.test(locSpan.textContent.trim()) ? locSpan.textContent.trim() : '';
      const severity  = dotStyle.includes('warning') ? 'warning'
                      : dotStyle.includes('error')   ? 'error'
                      : 'info';
...
(async () => {
  ...
  console.log('⏸  Browser is open. Click the "New Agent" button (top right) to open the editor...');
  ...
  await page.evaluate(() => new Promise(resolve => {
    if (window.__monaco__) { resolve(); return; }
    if (typeof require === 'function') {
      try {
        require(['vs/editor/editor.main'], (m) => {
          if (m) window.__monaco__ = m.editor ? m : { editor: m };
          resolve();
        });
  ...
  for (const filePath of agentFiles) {
    ...
    const setMethod = await page.evaluate((text) => {
      ...
      if (m) {
        ...
        if (models.length > 0) { models[0].setValue(text); return 'model-api'; }
        const editors = m.getEditors ? m.getEditors() : [];
        if (editors.length > 0) { editors[0].setValue(text); return 'editor-api'; }
      }
      return 'clipboard';
    }, content);
    ...
    for (let i = 0; i < 24; i++) {
      ...
      const count = await page.evaluate(() => {
        ...
        return Array.from(buttons).filter(btn =>
          Array.from(btn.querySelectorAll('span')).some(s => s.textContent.includes('-lint'))
        ).length;
      });
    ...
    const visibleCharCount = await page.evaluate(() => {
      ...
      return Array.from(lines).reduce((n, l) => n + l.textContent.length, 0);
    });

    if (visibleCharCount < 100 && actualContent === null) {
      console.log(`⚠️  SKIP (paste may have failed — only ${visibleCharCount} chars visible)`);
    ...
    if (markers === null) {
      ...
      const errors = domIssues.filter(i => i.severity === 'error').length;
      const warns  = domIssues.filter(i => i.severity === 'warning').length;
      if (domIssues.length === 0) {
        ...
        for (const issue of domIssues) {
          ...
          console.log(`${icon} [${issue.loc}] ${issue.message}  (${issue.rule})`);
        }
      }
  ...
  const totalErrors   = validated.reduce((n, r) => n + r.markers.filter(m => m.severity === 'error').length, 0);
  const totalWarnings = validated.reduce((n, r) => n + r.markers.filter(m => m.severity === 'warning' || m.severity === 'info').length, 0);
  const cleanFiles    = validated.filter(r => r.markers.length === 0).length;

  console.log('\n─────────────────────────────────────────');
  console.log(`Validated ${validated.length} file(s): ${cleanFiles} clean, ${validated.length - cleanFiles} with issues`);
  if (skipped.length > 0) {
  ...
  if (skipped.length === 0 && totalErrors === 0 && totalWarnings === 0) console.log('All files passed validation ✅');
